...
public class MultipleDataSourceBeanDefinitionRegistryPostProcessor implements BeanDefinitionRegistryPostProcessor,EnvironmentAware {
	...
	@Override public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry beanDefinitionRegistry)
		...
		try {
			if(!dataSourceMap.isEmpty()){
				...
				for(Entry<String,Map<String,Object>> entry:dataSourceMap.entrySet()){
					...
					beanDefinitionRegistry(beanDefinitionRegistry, entry.getKey(),(Class<? extends DataSource>)Class.forName(type.toString()));
				}
			}
	 ...
	 * @param
	 */
	private void beanDefinitionRegistry(BeanDefinitionRegistry beanDefinitionRegistry, String key,
			Class<? extends DataSource> beanClass) {
		...
		// 可以自动生成name
		String beanName = (key != null ? key : this.beanNameGenerator.generateBeanName(abd, beanDefinitionRegistry));
